...
class StrategyBase(bt.Strategy):
    ...
    def short(self):
        ...
        if cfg[ENV] == DEVELOPMENT:
            self.log("Sell ordered: $%.4f" % self.data0.close[0])
            return self.sell()

        cash, value = self.broker.get_wallet_balance(COIN_TARGET)
        amount = value*0.99
        self.log("Sell ordered: $%.4f. Amount %.6f %s - $%.4f USDT" % (self.data0.close[0],
    ...
    def notify_trade(self, trade):
        ...
        self.log(colored('OPERATION PROFIT, GROSS %.4f, NET %.4f' % (trade.pnl, trade.pnlcomm), color), True)
    ...
    def log(self, txt, send_telegram=False, color=None):
        ...
        if send_telegram:
            send_telegram_message(self.cfg, txt)
